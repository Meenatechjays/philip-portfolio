...
export default function TimelinePage() {
  ...
  const firstCardYear = carouselCards[0].year;
  const timelineEndYear = firstCardYear - 1;
  
  // Generate all years from start to current card
  const allYears = Array.from(
    { length: firstCardYear + carouselCards.length - TIMELINE_START_YEAR },
  ...
  useEffect(() => {
    const handleWheel = (e) => {
      ...
      if (scrollingDown) {
        if (activeCardIndex < carouselCards.length - 1) {
          e.preventDefault();
  ...
  return (
    ...
    >
        ...
        <div className="absolute bottom-0 left-0 right-0 overflow-hidden" style={{ height: '100%' }}>
          ...
          >
            ...
            {allYears.map((year, index) => {
              const cardIndex = carouselCards.findIndex(card => card.year === year);
              ...
              return (
                <div key={year} className="flex items-center gap-6">
                  ...
                  {hasCard && (
                    ...
                    >
                      <TimelineCard card={carouselCards[cardIndex]} />
                    </motion.div>
                  )}
      ...
      <div className="fixed top-4 right-4 bg-black/80 backdrop-blur-sm p-3 rounded-lg text-xs font-mono text-white z-50">
        ...
        <div>Active Card: <span className="text-blue-400">{activeCardIndex + 1}/{carouselCards.length}</span></div>
        <div>Year: <span className="text-blue-400">{carouselCards[activeCardIndex].year}</span></div>
